...
| 1.2 | ✅ **SHIPPED 2026-05-01** — Per-tenant locale (timezone + calendar) stored in `Organization.settings`. GCC defaults: Asia/Riyadh + Gregorian. Switchable to Hijri (Umm al-Qura) and 7 GCC + 5 global TZ choices. Frontend `formatDate` uses native `Intl.DateTimeFormat`. `dir="auto"` applied to alert/case/comment/news user-content. Settings page picker + live preview. 8 integration tests passing. | 1d | Compliance reports cite Hijri dates in Saudi; minimal effort. |
...
| 1.6 | ✅ **SHIPPED 2026-05-01** — Arabic phishing analyzer with five detection families: homoglyphs (Cyrillic/Greek/Arabic-Indic), bidi-control overrides (RLO/LRO/etc.), mixed-script IDN domains, 12 GCC pretext catalogues, and homoglyph-resolved brand impersonation. `POST /api/v1/intel/phishing/analyze` exposes it. 33 tests passing including the canonical Hajj-Aramco-Cyrillic-а phish (confidence=1.0). | 2d | Native ME flavour competitors don't have. |
| 1.7 | ✅ **SHIPPED 2026-05-01** — five commercial-licensable feeds, all license-verified for SaaS/on-prem use, no operator API keys required to bootstrap. **A** CertStream/crt.sh (typosquat backbone, 30-min polling). **B** CIRCL OSINT MISP (Iran-nexus IOCs, 6h). **C** PhishTank + CERT.PL (phishing URLs, 6h). **D** GHSA + ExploitDB (CVE → weaponised PoC mapping, 24h). **E** abuse.ch SSLBL + JA3 (TLS C2 fingerprints, 6h). All wired into `src/feeds/scheduler.py`. 8 smoke tests against canned upstream payloads. | 3d | High-yield, low-friction. Verify every ToS with legal before shipping. |
...
| # | Item | Effort | Rationale |
|---|---|---|---|
| 2.1 | ✅ **SHIPPED 2026-05-01** — `src/integrations/opencti/projection.py` projects IOCs / actors / alerts / cases into a co-deployed OpenCTI as STIX 2.1 entities (with `x_opencti_argus_*` ext props for round-trip pivots); `fetch_neighbourhood` proxies graph reads into a normalised `{root, nodes, edges}` shape. 3 routes under `/intel/opencti/*`. 27/27 tests. (iframe embed + bidi sync deferred to P3.) | 10d | Closes the knowledge-graph gap. RF charges $150K/yr for STIX export — we'd give it free. |
...
| 2.3 | ✅ **SHIPPED 2026-05-01** — `src/intel/sigma_rules.py` + 6 backends pinned (Splunk SPL · Elastic Lucene + EQL · Sentinel KQL · M365Defender KQL · QRadar AQL). 4 routes under `/intel/sigma/*`. 13/13 tests. | 4d | "Executable intel" promise. |
| 2.4 | ✅ **SHIPPED 2026-05-01** — Opt-in wrapper in `src/intel/kestrel_hunt.py` with pure render + subprocess execute paths. Hard-pinned stix-shifter==6.2.1 dep keeps Kestrel out of the main install; `ARGUS_KESTREL_CLI` points at a sibling-venv binary. 3 routes under `/intel/kestrel/*`. 15/15 tests. | 4d | Mandiant charges $400/hr consulting; we ship the hunt itself. |
| 2.5 | ✅ **SHIPPED 2026-05-01** — `src/intel/stix_shifter.py` wrapping stix-shifter 8.1 with Splunk + Elastic ECS + QRadar + Azure Sentinel modules pinned. IOC → STIX pattern → per-module query in one call. 3 routes under `/intel/stix-shifter/*`. 19/19 tests. | 3d | Pairs with Kestrel + Sigma. |
...
| 2.7 | ✅ **SHIPPED 2026-05-01** — `src/integrations/siem/` framework + 4 connectors. Splunk HEC NDJSON push. Sentinel (oauth Logs Ingestion API + legacy token webhook). Elastic ECS-shaped bulk index. QRadar reference-set bulk load. All gated by `http_circuit`; graceful no-op when unconfigured. 3 routes under `/intel/siem/*` (connectors / health / push). 21/21 tests. | 10d | 80 % dealbreaker. |
...
| 2.9 | ✅ **SHIPPED 2026-05-01** — `src/intel/attribution.py` ranks candidate actors per alert with a 5-factor weighted score (direct sighting · TTP overlap · IOC overlap · recency · infrastructure cluster — apex domain + /24 match). Per-factor breakdown shipped in the response. `GET /alerts/{id}/attribution` route. 12/12 tests. | 4d | "85 % likely vs 40 % likely" — a CISO question RF answers poorly on niche actors. |
| 2.10 | ✅ **SHIPPED 2026-05-01** — `src/intel/yarax_capa.py` exposes `yara-x` (Rust ~5x faster) for new scan paths and Mandiant `capa` for capability extraction with ATT&CK + MBC mapping. 50 MB sample ceiling. 3 routes under `/intel/yara/*` and `/intel/capa/*`. Existing yara-python paths unchanged. 13/13 tests. | 4d | Faster scans + replicated capability without bundling AGPL. |
| 2.11 | ✅ **SHIPPED 2026-05-01** — `src/integrations/misp.py` uses PyMISP to fetch events / attributes / galaxy clusters from a customer-configured MISP server. AGPL server intentionally not bundled. 4 routes under `/intel/misp/*`. 12/12 tests. | 3d | Sharing communities + galaxy clusters. |
...
| # | Item | Effort | Rationale |
|---|---|---|---|
| 3.1 | **Multi-tenant / MSSP mode** — full org isolation across DB (RLS already partial — extend to every table), API (JWT audience + per-org scopes), Redis (namespaced keys), MinIO (bucket-per-tenant), Meilisearch (index-per-tenant). MSSP "super-tenant" view aggregates across child tenants. **DROPPED** — go-to-market is per-customer on-prem (or per-customer cloud); the deployment IS the tenant boundary, so in-app multi-tenancy is overhead with no buyer. MSSP is a future business decision; if/when an MSSP deal materialises we'll scope to that buyer's actual ask, not to a speculative spec. The codebase already operates single-tenant via `get_system_org_id(db)`. | 12d | 80 % dealbreaker for SI resale. |
...
| 3.4 | **Customer-facing OpenAPI 3.1 schema** + **Python SDK** + **JS/TS SDK** + per-tenant TAXII publish (so the customer's downstream Splunk ES / Anomali / ThreatConnect can subscribe to *Argus-as-a-feed*). Per-user feed subscriptions + filter rules. **DONE** — TAXII 2.1 publish shipped earlier (32 tests). OpenAPI 3.1 schema served at /openapi.json (256 paths · 303 schemas) + checked in at clients/openapi/argus.openapi.json + scripts/export_openapi.py for offline regeneration. Hand-written Python SDK at clients/python/argus_sdk (sync + async, alerts/iocs/feeds/subscriptions/intel resources, 13 tests via httpx ASGITransport). Hand-written TS SDK at clients/typescript/argus-sdk (zero runtime deps, fetch-based, 9 unit tests via node:test). Feed subscriptions: per-user CRUD + filter matcher (severity/category/tags/min_confidence/title regex) + dry-run test endpoint, alembic-migrated, 20 tests. **65 tests pass**. | 8d | 65 % dealbreaker; TAXII publish alone is RF's $150K/yr feature. |
...
| 3.10 | **Telegram collector** with proper legal review (Telethon-based; tracks Iranian/Arabic ransomware + hacktivist channels). Persian/Arabic NLP + LLM IOC extraction in pipeline. **DONE** — Telethon transport behind opt-in legal-acknowledgment gate (ARGUS_TELEGRAM_ENABLED + API_ID + API_HASH + SESSION_PATH), 10-channel curated Iranian/Arabic catalog (Cyber Av3ngers, Predatory Sparrow, Moses Staff, MuddyWater, AnonGhost, Yemeni Cyber Army, …), Unicode-block Arabic/Persian language detection, IOC + category + victim-callout pipeline reusing the canonical IOC extractor, admin-gated fetch/health routes, 31 tests passing. | 8d | High ME signal (MuddyWater, Predatory Sparrow, Cyber Av3ngers). Phase 3 because legal review takes time. |
...
| 3.12 | **Mobile app companion** (read-only) — push alerts for Critical severity, biometric auth, alert detail view. iOS + Android via React Native or Capacitor. **DROPPED** — sales theatre, not an operational need. Real SOC analysts work at multi-monitor desks and never use mobile apps for investigation. CISOs / executives already get push from email + Slack/Teams + PagerDuty/Opsgenie (which have polished mobile apps). RF / Mandiant / CrowdStrike don't lead with their mobile apps — they're afterthoughts. If a customer specifically asks, a thin Capacitor wrapper around the dashboard is a 2–3d custom deliverable; we don't build it speculatively. | 10d | Executive-visibility — buyers love seeing alerts on their phone. Optional. |
